...
db.serialize(() => {
  ...
  const stmt = db.prepare('INSERT INTO photos (id, title, category, location, url, sizeClass, settings) VALUES (?, ?, ?, ?, ?, ?, ?)');
  for (const photo of defaultPhotos) {
    stmt.run(photo.id, photo.title, photo.category, photo.location, photo.url, photo.sizeClass, photo.settings);
  }
  stmt.finalize();
